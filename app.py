...
def limpiar_archivos_huerfanos():
    ...
    try:
        if not os.path.exists(app.config['UPLOAD_FOLDER']):
            print(f"Directorio de uploads no existe: {app.config['UPLOAD_FOLDER']}")
            return

        logos_en_uso = []
        try:
            resultados = ejecutar_consulta("SELECT imagen_logo FROM linea WHERE imagen_logo IS NOT NULL AND imagen_logo != ''")
            if resultados is None:
                print("Advertencia: La consulta de logos devolvió None, saltando limpieza")
                return
                
            logos_en_uso = [logo['imagen_logo'] for logo in resultados] if resultados else []
            print(f"Logos en uso encontrados: {len(logos_en_uso)}")
            
        except Exception as e:
            print(f"Error al obtener logos en uso: {e}")
            return

        archivos_en_directorio = os.listdir(app.config['UPLOAD_FOLDER'])
        print(f"Archivos en directorio: {len(archivos_en_directorio)}")
        
        archivos_eliminados = 0
        for archivo in archivos_en_directorio:
            if os.path.isfile(os.path.join(app.config['UPLOAD_FOLDER'], archivo)):
...
def api_conductores():
    ...
    conductores = ejecutar_consulta("""
        ...
        FROM conductor c 
        INNER JOIN vehiculo v ON c.cedula = v.conductor_ci
        LEFT JOIN linea l ON v.linea_asignada = l.id
        WHERE v.matricula IS NOT NULL AND v.matricula != ''
    """)
    
    print(f"Conductores recuperados: {len(conductores) if conductores else 0}")
    if conductores:
        for conductor in conductores:
            print(f"Conductor {conductor['cedula']} - Matrícula: {conductor.get('vehiculo_matricula', 'No disponible')}")
...
def api_conductor(cedula):
    ...
    print(f"Conductor {cedula} - Matrícula: {conductor.get('vehiculo_matricula', 'No disponible')}")
...
def api_crear_conductor():
    ...
    if data.get('matricula') and data.get('marca'):
        ...
        """, (
            ...
            print(f"Error al registrar vehículo para conductor {data['cedula']}")
        else:
            print(f"Vehículo registrado exitosamente: {matricula}")
...
def api_viaje(id):
    ...
    viaje = ejecutar_consulta("""
        SELECT v.*, 
               ...
               c.cedula,
               vh.marca,
               vh.matricula AS matricula_vehiculo,  -- CAMBIO IMPORTANTE: alias correcto
               vh.modelo,
               vh.linea_asignada,
               vh.capacidad
        FROM viaje v
